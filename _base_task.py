...
import glob
import sys
sys.path.append('/root/autodl-tmp/RoboTwin/policy/my_DP3/3D-Diffusion-Policy')
from ._GLOBAL_CONFIGS import *
from envs.camera.camera import fps 
...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        try:
            ...
            # 换成你真实的权重路径
            self.semantic_extractor = SemanticPointExtractor('/root/autodl-tmp/RoboTwin/policy/my_DP3/3D-Diffusion-Policy/checkpoints/dinov2_linear_head.pth')
            print("👁️ 数据采集器已成功挂载 DINOv2 语义过滤基座！")
        except ImportError:
            self.semantic_extractor = None
            print("⚠️ 未找到 DINOv2，将采集原始环境点云。")
    ...
    def get_obs(self, semantic_mask=None, update=True):
        self._update_render()
        ...
        if self.data_type.get("pointcloud", False):
            pkl_dic["pointcloud"] = self.cameras.get_pcd(self.data_type.get("conbine", False), semantic_mask=semantic_mask)
    ...
    def _take_picture(self):  # save data
        ...
        if hasattr(self, 'semantic_extractor') and self.semantic_extractor is not None:
            ...
            # 取出头部相机的 RGB 数组 (默认是 numpy array)
            rgb_img = rgb_dict['head_camera']['rgb'] 
            
            # 预测掩膜
            semantic_mask = self.semantic_extractor.predict(rgb_img)

        # 带着掩膜，只执行唯一一次完整的观测收集！
        pkl_dic = self.get_obs(semantic_mask=semantic_mask, update=False)
        # =================================================================
        # 🌟 2. 注入漏斗降采样逻辑 (FPS + 重采样)
        # =================================================================
        if self.data_type.get("pointcloud", False) and "pointcloud" in pkl_dic:
            pcd = pkl_dic["pointcloud"]
            target_num = 1024  # 你训练数据需要的点数
            
            if pcd.shape[0] == 0:
                print(f"\n⚠️ 警告: 帧 {self.FRAME_IDX} 未提取到目标，使用零填充兜底！")
                pcd_fixed = np.zeros((target_num, pcd.shape[1]), dtype=np.float32)
                
            elif pcd.shape[0] < target_num:
                # 匮乏分支：有放回重采样
