...
class TrainDP3Workspace:
    ...
    def run(self):
        ...
        # [修改：移除硬编码] 改为从 config 中读取 WandB 开关
        WANDB = cfg.logging.get('mode') == 'online' if hasattr(cfg, 'logging') else False
        ...
        # [修改：移除硬编码] 让这些行为听从 yaml 配置
        RUN_VALIDATION = cfg.training.val_every > 0
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0 and cfg.checkpoint.save_ckpt:
                ...
                if not cfg.policy.use_pc_color:
                    ckpt_dir = f"checkpoints/{self.cfg.task.name}_{cfg.training.seed}"
                else:
                    ckpt_dir = f"checkpoints/{self.cfg.task.name}_w_rgb_{cfg.training.seed}"
                    
                if not os.path.exists(ckpt_dir):
                    os.makedirs(ckpt_dir)
                    
                save_path = f"{ckpt_dir}/{self.epoch + 1}.ckpt"
                self.save_checkpoint(save_path)
                
                # [修改：启用 TopK 管理器] 根据验证集 Loss 自动清理多余的旧模型，防止硬盘塞满
                if "val_loss" in step_log:
                    val_score = -step_log["val_loss"] 
                    topk_manager.update(step=self.epoch, score=val_score, ckpt_path=save_path)

            # ========= eval end for this epoch ==========
            policy.train()

            # [新增：本地 CSV 保存 Loss] 每次验证结束时，将关键指标追加写入 CSV 文件
            import csv
            csv_log_path = os.path.join(self.output_dir, "loss_log.csv")
            file_exists = os.path.isfile(csv_log_path)
            with open(csv_log_path, mode='a', newline='') as f:
                fieldnames = ["epoch", "global_step", "lr", "train_loss", "val_loss"]
                ...
                writer.writerow({
                    "epoch": self.epoch,
                    "global_step": self.global_step,
                    "lr": step_log.get("lr", 0),
                    "train_loss": step_log.get("train_loss", 0),
